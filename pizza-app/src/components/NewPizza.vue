<template>
  <form>
    ...
    <div class="form-group row">
      <button type="button" @click="addPizzaItem" class="btn btn-success btn-block">添加</button>
    </div>
  </form>
</template>
<script>
export default {
  ...
  methods: {
    addPizzaItem() {
      ...
      this.axios
        .post("/menu.json", pizzaData)
        .then(response => {
          console.log(response.data);
        })
        //数据同步到vuex中
        .then(res => this.$store.commit("pushToMenuItems", pizzaData));
    }
  }
};
</script>